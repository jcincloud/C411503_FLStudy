	...
	render:function(){
		return (
				...
				<tr>
					...
					<td>{this.props.itemData.video_title}</td>
					<td>{this.categoryVFilter(this.props.itemData.category)}</td>
					<td>{this.props.itemData.video_url}</td>
...
var GirdForm = React.createClass({
	...
	getInitialState: function() {  
		return {
			...
			fieldData:{},
			searchData:{name:null,category:gb_category},
			edit_type:0,
			checkAll:false,
			category:[]
		};  
	...
	getDefaultProps:function(){
		return{	
			fdName:'fieldData',
			gdName:'searchData',
			apiPathName:gb_approot+'api/Video',
			initPathName:gb_approot+'Active/Video/aj_Init'
	...
	deleteSubmit:function(e){
		...
		for(var i in this.state.gridData.rows){
			if(this.state.gridData.rows[i].check_del){
				ids.push('ids='+this.state.gridData.rows[i].video_id);
			}
		}
	...
	insertType:function(){
		this.setState({edit_type:1,fieldData:{category:gb_category,i_Hide:false,i_Lang:'zh-TW'}});
	},
	...
	getAjaxInitData:function(){
		...
		.done(function(data, textStatus, jqXHRdata) {
			this.setState({category:data.options_video_category});
			//載入下拉是選單內容
		}.bind(this))
		.fail(function( jqXHR, textStatus, errorThrown ) {
			showAjaxError(errorThrown);
						...
						<table>
							<thead>
								<tr>
									...
									<th className="col-xs-2">影片標題</th>
									<th className="col-xs-2">分類</th>
									<th className="col-xs-3">影片連結</th>
			...
			<div>
				...
				<div className="form-group">
						...
						<div className="col-xs-6">
							<MasterImageUpload
							FileKind="Photo1" 
							MainId={fieldData.video_id}
							ParentEditType={this.state.edit_type}
							url_upload={gb_approot + 'Active/Video/axFUpload'}
							url_list={gb_approot+'Active/Video/axFList'}
							url_delete={gb_approot+'Active/Video/axFDelete'}
							url_sort={gb_approot+'Active/Video/axFSort'}
							/>
						</div>
					...
					<div className="form-group">
						<label className="col-xs-1 control-label text-danger">影片標題</label>
						<div className="col-xs-6">
							<input type="text" 
							className="form-control"	
							value={fieldData.video_title}
							onChange={this.changeFDValue.bind(this,'video_title')}
							maxLength="64"
							required />						
						</div>
					...
					<div className="form-group">
						<label className="col-xs-1 control-label text-danger">影片網址</label>
						<div className="col-xs-6">
							<input type="text" 
							className="form-control"	
							value={fieldData.video_url}
							onChange={this.changeFDValue.bind(this,'video_url')}
							maxLength="400"
							required />						
						</div>
						<small className="col-xs-5 help-inline">請輸入完整連結網址 (包含 http:// 開頭)</small>
					</div>
