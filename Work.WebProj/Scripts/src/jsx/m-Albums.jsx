var GridRow = React.createClass({
	...
	categoryYFilter:function(year){
		var val="";
		this.props.year.forEach(function(object, i){
        	if(year==object.val){
  				val=object.Lname;
        	}
    	})
	...
	render:function(){
		return (
				...
				<tr>
					...
					<td>{this.categoryYFilter(this.props.itemData.year)}</td>
...
var GirdForm = React.createClass({
	...
	getInitialState: function() {  
		return {
			...
			fieldData:{},
			searchData:{title:null,category:null,year:null},
			edit_type:0,
			checkAll:false,
			category:[],
			year:[]
	...
	getDefaultProps:function(){
		return{	
			fdName:'fieldData',
			gdName:'searchData',
			apiPathName:gb_approot+'api/Albums',
			initPathName:gb_approot+'Active/Albums/aj_Init'
	...
	handleSubmit: function(e) {
		...
		if(this.state.edit_type==1){
			...
			this.state.fieldData.albums_content = CKEDITOR.instances.editor1.getData();//編輯器
	...
	deleteSubmit:function(e){
		...
		for(var i in this.state.gridData.rows){
			if(this.state.gridData.rows[i].check_del){
				ids.push('ids='+this.state.gridData.rows[i].albums_id);
			}
		}
	...
	insertType:function(){
		...
		var defaultY=this.state.year;

		this.setState({edit_type:1,fieldData:{category:defaultC[0].val,year:defaultY[0].val}});
	},
	...
	getAjaxInitData:function(){
		...
		.done(function(data, textStatus, jqXHRdata) {
			this.setState({category:data.options_category,year:data.options_year});
			//載入下拉是選單內容
		}.bind(this))
		.fail(function( jqXHR, textStatus, errorThrown ) {
			showAjaxError(errorThrown);
	...
	changeLangVal:function(e){
		...
		CArray=this.getLangCArray(e.target.value);
		YArray=this.getLangYArray(e.target.value);

		this.setState({fieldData:obj,c_Category:CArray,c_Year:YArray});
	},
	getLangCArray:function(lang){
		var ArrayVal=[];
		this.state.langCategory.forEach(function(object, i){
        	if(lang==object.lang){
	...
	getLangYArray:function(lang){
		var ArrayVal=[];
		this.state.langYear.forEach(function(object, i){
        	if(lang==object.lang){
  				ArrayVal=object.items;
        	}
    	})
							...
							<div className="table-filter">
								<div className="form-inline">
									<div className="form-group">
										...
										{
											this.state.year.map(function(itemData,i) {
				...
				<form className="form-horizontal" onSubmit={this.handleSubmit}>
					...
					<div className="form-group">
						...
						<div className="col-xs-7">
							<MasterImageUpload 
							FileKind="Photo1" 
							MainId={fieldData.albums_id}
							ParentEditType={this.state.edit_type}
							url_upload={gb_approot + 'Active/Albums/axFUpload'}
							url_list={gb_approot+'Active/Albums/axFList'}
							url_delete={gb_approot+'Active/Albums/axFDelete'}
							url_sort={gb_approot+'Active/Albums/axFSort'}
							/>
						</div>
					...
					<div className="form-group">
						<label className="col-xs-1 control-label">年度</label>
						<div className="col-xs-2">
							<select className="form-control" value={fieldData.year} 
							onChange={this.changeFDValue.bind(this,'year')}
							required>
							{
								this.state.year.map(function(itemData,i) {
								return <option key={i} value={itemData.val}>{itemData.Lname}</option>
						...
						<label className="col-xs-1 control-label">活動名稱</label>
					...
					<div className="form-group">
						<label className="col-xs-1 control-label">相片集錦</label>
						<div className="col-xs-7">
							<MasterImageUpload 
							FileKind="Photo2" 
							MainId={fieldData.albums_id}
						...
						<small className="help-inline col-xs-4 text-danger">最多 20 張圖片</small>
					</div>

					<div className="form-group">
						<label className="col-xs-1 control-label">相簿連結</label>
							<div className="col-xs-7">
								<textarea type="date" className="form-control" rows="1"
									value={fieldData.url}
									onChange={this.changeFDValue.bind(this,'url')}
									maxLength="400"/>
								<small className="help-block">請輸入包含 http:// 開頭的完整連結網址 ex. FB 或部落格的相簿網址 (此欄非必要)</small>
							</div>
					</div>

					<div className="form-group">
						<label className="col-xs-1 control-label">內容簡介</label>
							<div className="col-xs-10">
								<textarea type="date" className="form-control" rows="3" id="editor1"
									value={fieldData.albums_content}
									onChange={this.changeFDValue.bind(this,'albums_content')} />
							</div>
					</div>
