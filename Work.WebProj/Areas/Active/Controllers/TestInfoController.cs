...
namespace DotWeb.Areas.Sys_Active.Controllers
{
    public class TestInfoController : BaseController
    {
        ...
        public string aj_Init()
        {
            ...
            {
                var options_category = db0.All_Category_L2.Where(x => x.all_category_l1_id == CategoryType.TestInfo && x.i_Hide == false).OrderBy(x => x.sort).Select(x => new option() { val = x.all_category_l2_id, Lname = x.l2_name }).ToList();
        ...
        public string axFUpload(int id, string filekind, string filename)
        {
            ...
            try
            {
                ...
                //代表 圖片
                if (filekind == "Photo1")
                    handleImageSave(filename, id, ImageFileUpParm.Photo, filekind, "TestInfo", "Photo");
        ...
        public string axFList(int id, string filekind)
        {
            ...
            if (filekind == "Photo1")
                r.files = listImgFiles(id, filekind, "TestInfo", "Photo");
        ...
        public string axFSort(int id, string filekind, IList<JsonFileInfo> file_object)
        {
            ...
            if (filekind == "Photo1")
                rewriteJsonFile(id, filekind, "TestInfo", "Photo", file_object);
        ...
        {
            string path_tpl = string.Format(upload_path_tpl_o, "TestInfo", "Photo", id, filekind, filename);
