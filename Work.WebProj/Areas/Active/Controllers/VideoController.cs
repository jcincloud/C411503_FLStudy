...
namespace DotWeb.Areas.Sys_Active.Controllers
{
    public class VideoController : BaseController
    {
        ...
        public string aj_Init()
        {
            ...
            {
                var options_video_category = db0.All_Category_L2.Where(x => x.all_category_l1_id == CategoryType.Video && x.i_Hide == false).OrderBy(x => x.sort).Select(x => new option() { val = x.all_category_l2_id, Lname = x.l2_name }).ToList();
                if (System.Globalization.CultureInfo.CurrentCulture.Name == "zh-CN")
                {
                    foreach (var i in options_video_category)
                    {
                ...
                return defJSON(new
                {
                    options_video_category = options_video_category
                });
            }
        }
        #endregion
        ...
        public string axFUpload(int id, string filekind, string filename)
        {
            ...
            try
            {
                //代表 圖片
                if (filekind == "Photo1")
                    handleImageSave(filename, id, ImageFileUpParm.Photo, filekind, "Video", "Photo");
        ...
        public string axFList(int id, string filekind)
        {
            ...
            if (filekind == "Photo1")
                r.files = listImgFiles(id, filekind, "Video", "Photo");
        ...
        public string axFDelete(int id, string filekind, string filename)
        {
            ...
            if (filekind == "Photo1")
                DeleteSysFile(id, filekind, filename, ImageFileUpParm.Photo, "Video", "Photo");
        ...
        public string axFSort(int id, string filekind, IList<JsonFileInfo> file_object)
        {
            ResultInfo r = new ResultInfo();
            if (filekind == "Photo1")
                rewriteJsonFile(id, filekind, "Video", "Photo", file_object);
        ...
        {
            string path_tpl = string.Format(upload_path_tpl_o, "Video", "Photo", id, filekind, filename);
