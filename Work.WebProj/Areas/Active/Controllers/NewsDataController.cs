...
{
    public class NewsDataController : BaseController
    {
        ...
        public string axFUpload(int id, string filekind, string filename)
        {
            ...
            try
            {
                //內頁 圖片
                if (filekind == "Photo2")
                    handleImageSave(filename, id, ImageFileUpParm.Photo, filekind, "News", "Photo");
        ...
        public string axFList(int id, string filekind)
        {
            ...
            if (filekind == "Photo1")
                r.files = listImgFiles(id, filekind, "News", "Photo");
        ...
        public string axFDelete(int id, string filekind, string filename)
        {
            ...
            if (filekind == "Photo2")
                DeleteSysFile(id, filekind, filename, ImageFileUpParm.Photo, "News", "Photo");
        ...
        public string axFSort(int id, string filekind, IList<JsonFileInfo> file_object)
        {
            ...
            if (filekind == "Photo1")
                rewriteJsonFile(id, filekind, "News", "Photo", file_object);
        ...
        {
            string path_tpl = string.Format(upload_path_tpl_o, "News", "Photo", id, filekind, filename);
