...
namespace DotWeb.Areas.Sys_Active.Controllers
{
    public class StudyAbroadDataController : BaseController
    {
        ...
        public string aj_Init()
        {
            ...
            {
                var options_vacation_category = db0.All_Category_L2.Where(x => x.all_category_l1_id == CategoryType.Vacation_2 && x.i_Hide == false).OrderByDescending(x => x.sort).Select(x => new option() { val = x.all_category_l2_id, Lname = x.l2_name }).ToList();
