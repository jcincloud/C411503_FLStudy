    ...
    public class AlbumsController : BaseController
    {
        ...
        public string axFUpload(int id, string filekind, string filename)
        {
            ...
            try
            {
                //代表 圖片
                if (filekind == "Photo1")
                    handleImageSave(filename, id, ImageFileUpParm.Photo, filekind, "Albums", "Photo");

                //相片集錦 圖片
                if (filekind == "Photo2")
                    handleImageSave(filename, id, ImageFileUpParm.PhotoCollectionB, filekind, "Albums", "Photo");
                r.result = true;
        ...
        public string axFDelete(int id, string filekind, string filename)
        {
            ...
            if (filekind == "Photo1")
                DeleteSysFile(id, filekind, filename, ImageFileUpParm.Photo, "Albums", "Photo");


            if (filekind == "Photo2")
                DeleteSysFile(id, filekind, filename, ImageFileUpParm.PhotoCollectionB, "Albums", "Photo");
